...
if (typeof module !== "undefined" && module.exports) {
  ...
  const nodeRootDir = typeof __dirname !== "undefined" ? __dirname : process.cwd();
  const nodeMimeTypes = {
  ...
  async function serveStaticAsset(request, response) {
    ...
    const filePath = path.resolve(nodeRootDir, relativePath);
    if (!filePath.startsWith(nodeRootDir)) {
    ...
    try {
      ...
      response.setHeader("content-type", nodeMimeTypes[ext] || "application/octet-stream");
      response.end(body);
    } catch {
      try {
        const body = await fs.readFile(path.join(nodeRootDir, "index.html"));
        response.statusCode = 200;
        response.setHeader("content-type", nodeMimeTypes[".html"]);
        response.end(body);
      } catch (error) {
  ...
  module.exports = async function appHandler(request, response) {
    const url = new URL(request.url, "http://localhost");
